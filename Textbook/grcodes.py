...
def solver1D2(E, A, bf, l, bc0, bcl, key='disp'):
    '''Solves 1D mechanics prolems for 1D bars subjected to body 
    force bf: u,xx=-bf(x)/E, with BCs: u(0)=bc0, u(l)=bcl or fl=bcl.
    Input: bf, body force; l, the length of the bar; 
           bcl, condition at x=l; key="disp", displacement condition;
           otherwise force condition
    Return: u, u_x, u_xx ''' 
    ...
    u_xx = sp.diff(u_x,x)
    print(f'output from solver1D2(): u, u_x, u_xx')
    return u.expand(), u_x.expand(), u_xx.expand()
